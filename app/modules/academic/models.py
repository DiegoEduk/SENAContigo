...
class ProgramaFormacion(Base):
    ...
    codigo_programa: Mapped[str] = mapped_column(String(50), unique=True, index=True, nullable=False)
...
class Ficha(Base):
    ...
    centro_id: Mapped[int] = mapped_column(ForeignKey("centros.id", ondelete="CASCADE"), nullable=False)
    programa_id: Mapped[int] = mapped_column(ForeignKey("programas.id", ondelete="CASCADE"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
